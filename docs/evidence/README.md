...
Runs that published writing quotes. Each was captured from a real run by `tools/capture_evidence.py`, which refuses to run on a dirty working tree, so the commit named in a file is the code that produced its output. The files record one run each and are not regenerated; a later run prints a different run id.

Both files below were captured on 2026-09-17 at commit `48ca993`, for `cordata-tech/platform#49` § 13 to quote, as the captured-run acceptance item on [#1](https://github.com/cordata-tech/pipeline-runtime/issues/1) asks.
